...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['photo']) && $_FILES['photo']['error'] === UPLOAD_ERR_OK) {
        ...
        // Validate file type and size (example: allow only JPEG and PNG, max size 5MB)
        $validImageTypes = ['image/jpeg', 'image/png'];
        if (!in_array($_FILES['photo']['type'], $validImageTypes)) {
            echo json_encode(array('status' => 'error', 'message' => 'Invalid photo format.'));
            exit;
        }
        if ($_FILES['photo']['size'] > 5 * 1024 * 1024) {  // 5MB limit
            echo json_encode(array('status' => 'error', 'message' => 'Photo size exceeds the maximum limit.'));
            exit;
        }
    ...
    if (isset($_FILES['video']) && $_FILES['video']['error'] === UPLOAD_ERR_OK) {
        ...
        // Validate file type and size (example: allow only MP4, max size 50MB)
        $validVideoTypes = ['video/mp4'];
        if (!in_array($_FILES['video']['type'], $validVideoTypes)) {
            echo json_encode(array('status' => 'error', 'message' => 'Invalid video format.'));
            exit;
        }
        if ($_FILES['video']['size'] > 50 * 1024 * 1024) {  // 50MB limit
            echo json_encode(array('status' => 'error', 'message' => 'Video size exceeds the maximum limit.'));
            exit;
        }
