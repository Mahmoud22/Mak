...
<form method="POST" action="insert_form_pc.php" id="dynamicForm" enctype="multipart/form-data">
                    <!-- Hidden Inputs for client_id and project_id -->
...
<script>
// Dynamically set the hidden fields based on user input or selection
document.getElementById('dynamicForm').addEventListener('submit', function(event) {
    // Ensure that hidden fields are correctly populated
    const clientInput = document.getElementById('client');
    const projectInput = document.getElementById('project');

    const clientId = getClientIdFromName(clientInput.value);
    const projectId = getProjectIdFromName(projectInput.value);

    // Check if both client_id and project_id are properly populated
    if (!clientId || !projectId) {
        event.preventDefault();
        alert('Please ensure the client and project are selected!');
        return; // Prevent submission
    }

    // Set the hidden inputs with the appropriate values
    document.getElementById('client_id').value = clientId;
    document.getElementById('project_id').value = projectId;

    console.log('Before Submit: client_id =', clientId, ', project_id =', projectId);  // Debugging line
});

// Function to simulate client ID retrieval (Replace with real logic or AJAX calls)
function getClientIdFromName(clientName) {
    // Example: Replace with actual client retrieval logic
    if (clientName === 'John Doe') return 'gg';  // Assuming 'gg' is the client ID
    return '';  // Default: return empty if no client selected
}

// Function to simulate project ID retrieval (Replace with real logic or AJAX calls)
function getProjectIdFromName(projectName) {
    // Example: Replace with actual project retrieval logic
    if (projectName === 'Project A') return 'mm';  // Assuming 'mm' is the project ID
    return '';  // Default: return empty if no project selected
}
